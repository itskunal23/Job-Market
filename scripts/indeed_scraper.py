"""
Indeed Job Scraper using Guest Portal
Uses Playwright to scrape Indeed jobs without authentication
"""
...
class IndeedScraper:
    """Scrape jobs from Indeed Guest Portal using Playwright"""
            ...
            with sync_playwright() as p:
                ...
                for selector in job_card_selectors:
                    try:
                        ...
                        if len(cards) > 0:
                            job_cards = cards
                            print(f"Indeed: Found {len(cards)} jobs using {selector}")
                            break
    ...
    def _extract_job_from_card(self, card, page) -> Optional[JobPosting]:
        ...
        try:
            # Title and URL
            title_selectors = ['h2.jobTitle a', 'a[data-jk]', 'h2 a']
            title_element = None
            ...
            # Company
            company_selectors = ['span[data-testid="company-name"]', '.companyName', 'span.companyName']
            ...
            # Location
            location_selectors = ['div[data-testid="text-location"]', '.companyLocation', 'div.companyLocation']
            ...
            # Salary
            salary_element = card.query_selector('span[data-testid="attribute_snippet_testid"], .salary-snippet')
            salary = salary_element.inner_text().strip() if salary_element else None
            
            # Description snippet
            desc_element = card.query_selector('.job-snippet, div[data-testid="job-snippet"]')
            description = desc_element.inner_text().strip() if desc_element else ""
            
            # Posted date
            date_element = card.query_selector('span[data-testid="myJobsStateDate"], .date')
            posted_date = None
            if date_element:
    ...
    def _generate_mock_jobs(self, keywords: List[str], location: str, count: int) -> List[JobPosting]:
        ...
        for i in range(min(count, 15)):
            jobs.append(JobPosting(
                ...
                url=f"https://www.indeed.com/viewjob?jk={i+1}",
                platform="Indeed",
